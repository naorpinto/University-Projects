##------------------------------------------Alternative 1-----------------------------------------------------------------
#Shortening test time in the test stations-- from 1.5 Minutes to 1 Minute 
#Adding ambulance
#Adding an employee to one of the labs-lab  
...
Covid19<- simmer("Covid19")%>%
  ...
  add_resource("MDA",capacity=21,queue_size=Inf,preemptive = FALSE)%>% 
  ...
  add_resource("Lab_C",capacity=21,queue_size=Inf,preemptive = FALSE)%>%
...
MDA <- trajectory("MDA")%>%
  ...
  timeout(function() rtriangle(1,10,20,15) + 1)%>% ##the time it takes to drive to the client and test him for corona
  release("MDA", amount =1)%>%
...
mm2envs <- mclapply(1:15, function(i) {                    #when we run the  simulation 15 replication
  set.seed(248+i)
...
arrivalData <- get_mon_arrivals(mm2envs)
resourceData <- get_mon_resources(mm2envs)
attributeData <- get_mon_attributes(mm2envs)
...
mda_table <- sqldf("select * from resourceData as re
  where re.resource in ('MDA') and re.time>=warming_time" )     #Taking the relevant data after the warming time
...
for (i in 1:15) {                                                          #when we run the  simulation 15 replication
  ...
  mesuresMDAQ_alt1[i] <- avgQueue(time, queueLength, simulationTime-warming_time)
}
print(mesuresMDAQ_alt1)
Mean1_alternative1_MDA_Q <- mean(mesuresMDAQ_alt1)
sd1_alternative1_MDA_Q <- sd(mesuresMDAQ_alt1)
...
wt_hmo_alt1 <- sqldf("select replication, avg(waiting_time) as wt from (
  select fd.replication , (time-start_time-1.5) as waiting_time from arrivalData as fd join attributeData as ad 
                ...
                group by replication")                                       #Taking the relevant data after the warming time            


Mean2_alternative1_WT <- mean(wt_hmo_alt1$wt)
sd2_alternative1_WT <- sd(wt_hmo_alt1$wt)
...
for (i in 1:15) {                                                                    #when we run the  simulation 15 replication
  ...
  mesuresLabQ_alt1[i] <- avgQueue(time, queueLength, simulationTime-warming_time)
}

Mean3_alternative1_LabC_Q <- mean(mesuresLabQ_alt1)
sd3_alternative1_LabC_Q  <- sd(mesuresLabQ_alt1)
#-----------------------------------------------------------------------------------------------------------------------
alt1_measure1 <-mesuresMDAQ_alt1
alt1_measure2 <-wt_hmo_alt1$wt
alt1_measure3 <- mesuresLabQ_alt1
alt1_table <- as.matrix(data.frame(alt1_measure1,alt1_measure2,alt1_measure3))
write.csv(alt1_table,"C:/Users/naorp/Desktop/סימולציה/פרוייקט/Alt1Table_measures.csv")
