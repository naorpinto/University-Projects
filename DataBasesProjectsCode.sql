...
-----------------------------------------------------
select g.Email, f.ID_Product

from Guests as g join Searches as s on g.Email=s.Email join
	 Findings as f on f.IP_Search=s.IP_Search and f.DT_Search=s.DT_Search
where g.Email  not in (select Email from Shipping_Times as sh join Orders as o
						 on sh.ID_Order=o.ID_Order where f.ID_Product=ID_Product)
order by 2
...
having Stars<=(select avg (Stars) from Reviews)
...
CREATE FUNCTION f_checkOrder (@email varchar(50))
returns int as
begin
	declare @answer int

		if @email in(select email from orders) set @answer=1
		else set @answer=0
...
update guests 
set guests.Updates=dbo.f_checkOrder(@email)
